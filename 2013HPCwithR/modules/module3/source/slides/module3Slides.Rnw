...
\begin{frame}{MGHPCC}
\begin{itemize}
\item The Massachusetts Green High Performance Computing Center (MGHPCC) is one of these clusters. It currently has capacity for 10,000 nodes, with more space coming online soon.  
...
\begin{frame}{How to login using VPN}
\begin{itemize}
