...
\begin{frame}[fragile]
{\sc Excersie 1:} Conduct a simulation study to identify an appropriate p-value threshold for statistical significance (assuming independence of SNPs):
\begin{itemize}
  \item generate $965,220$ p-values from a uniform distribution
  \item determine value corresponding to the $5$th percentile
	\item repeat $500$ times and record $5$th percentile of this distn.
...
matNullP <- foreach(i=1:nSim, .combine=rbind) %dopar% {
        ...
        c(i, sort(sim)[nrow(cardioDat)*0.05])
}
